...
{status_html}
      ...
      <p class="floor-note">楼层分布中，已售为红色；可售为绿色并展示单价、总价；可售房源相同单价使用同一色块。</p>
...
ONE_PRICE_JS = r'''(function(){
  ...
  function floorKey(room){return [text(room.buildName),text(room.unitName),text(room.floor)].join('\\u0001');}
  ...
  function renderFloorView(filtered){
    ...
    filtered.forEach(function(room){
      var key=[text(room.buildName),text(room.unitName)].join('\\u0001');
      if(!groups.has(key))groups.set(key,{building:text(room.buildName),unit:text(room.unitName),byFloor:new Map()});
      var floors=groups.get(key).byFloor;var floor=text(room.floor);
      if(!floors.has(floor))floors.set(floor,[]);floors.get(floor).push(room);
    });
    Array.from(groups.values()).sort(function(left,right){return naturalCompare(left.building,right.building)||naturalCompare(left.unit,right.unit)}).forEach(function(group){
      var section=document.createElement('section');section.className='floor-group';section.setAttribute('aria-label',group.building+' '+group.unit+' 楼层分布');
      var head=document.createElement('div');head.className='floor-group-head';
      var title=document.createElement('h3');title.textContent=group.building+' · '+group.unit;head.appendChild(title);
      var size=document.createElement('span');size.textContent=Array.from(group.byFloor.values()).reduce(function(total,rooms){return total+rooms.length},0)+' 套';head.appendChild(size);
      section.appendChild(head);
      Array.from(group.byFloor.entries()).sort(function(left,right){var ln=floorNumber(left[0]);var rn=floorNumber(right[0]);if(isFinite(ln)&&isFinite(rn)&&ln!==rn)return rn-ln;if(isFinite(ln))return -1;if(isFinite(rn))return 1;return naturalCompare(left[0],right[0])}).forEach(function(entry){
        var floorRow=document.createElement('div');floorRow.className='floor-row';
        var label=document.createElement('div');label.className='floor-label';label.textContent=entry[0];floorRow.appendChild(label);
        var cells=document.createElement('div');cells.className='room-cells';
        entry[1].sort(function(left,right){return naturalCompare(left.roomName,right.roomName)}).forEach(function(room){cells.appendChild(roomCell(room))});
        floorRow.appendChild(cells);section.appendChild(floorRow);
      });
      floorResults.appendChild(section);
    });
  }
  function render(){
