...
ONE_PRICE_JS = r'''(function(){
  ...
  function renderFloorView(allRooms){
    ...
    if(columns.length>DETAIL_BUILDING_LIMIT){
      ...
      columns.forEach(function(column){
        ...
        block.appendChild(body);block.addEventListener('pointerup',function(event){if(event.pointerType==='touch'&&!gestureMoved)focusBuilding(column)});block.addEventListener('click',function(event){if(gestureMoved)return;focusBuilding(column)});overview.appendChild(block);
      });
      canvas.appendChild(overview);stage.appendChild(canvas);
    }else{
      ...
      canvas.appendChild(matrix);stage.appendChild(canvas);
    }
  ...
  loadJSON(url).then(function(data){
    ...
    floorResults.replaceChildren();var floorError=document.createElement('p');floorError.className='loading';floorError.textContent='楼层分布载入失败：'+error.message;floorResults.appendChild(floorError);if(backToOverview)floorResults.appendChild(backToOverview);
