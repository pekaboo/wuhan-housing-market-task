...
{status_html}
      ...
      <p class="floor-note">楼层分布按楼栋从左到右、楼层从上到下整体排布；已售为红色，可售为绿色并展示单价、总价；可售房源相同单价使用同一色块。</p>
...
ONE_PRICE_JS = r'''(function(){
  ...
  function renderFloorView(filtered){
    ...
    matrix.setAttribute('aria-label','楼栋从左到右、楼层从上到下的一房一价分布');
