...
def all_charts(project: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if project.get('soldChartImg'):
        charts.append(
            {
                'img': project['soldChartImg'],
                'time': project.get('date'),
                'title': '标准销控图',
            }
        )
    if project.get('douYinSoldChartImg'):
        charts.append(
            {
                'img': project['douYinSoldChartImg'],
                'time': project.get('date'),
                'title': '抖音版销控图',
            }
        )
...
<header class="hero">
  ...
  <p class="description">共 {all_count} 个楼盘中的当前项目详情页；展示接口返回的全部字段、标准销控图、抖音版销控图与历史销控图。</p>
</header>
...
<h2>全部字段</h2>
<section class="field-panel" aria-label="全部接口字段">
  <div class="field-wrap">
    <table class="field-table">
      <caption class="sr-only">接口返回的全部字段与值</caption>
      <thead><tr><th scope="col">字段</th><th scope="col">值</th></tr></thead>
      <tbody>{field_rows(project)}</tbody>
    </table>
  </div>
</section>
