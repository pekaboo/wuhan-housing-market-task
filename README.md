...
- **楼层分布**：按楼栋 / 单元从左到右、真实楼层从上到下排成整体矩阵，同一楼层的全部房源保持横向一排；空缺楼层格用 `—` 占位保持楼栋对齐；已售为红色，可售为绿色，并展示房号、户型、面积、单价与总价；可售房源相同单价使用同一个价格色块，价格文本与状态文本同时保留，避免只依赖颜色。窄屏时矩阵在卡片内横向滚动，页面本身不横向溢出。
